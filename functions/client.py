        ...
        while True:
            try:
                ...
                # Im process_messages, nach dem Handler für karte_abgelehnt:
                    ...
                    last_turn_players = msg["last_turn_players"]
                    trigger_name = s.player_data.get(trigger_player, f"Spieler {trigger_player}")

                    s.round_ending = True
                    s.round_end_trigger = trigger_player
                    s.last_turn_players = last_turn_players

                    # Passende Nachricht anzeigen
                    if s.spieler_id in last_turn_players:
                        s.status_message = f"{trigger_name} hat alle Karten aufgedeckt! LETZTER SPIELZUG FÜR DICH!"
                    else:
                        s.status_message = f"Du hast alle Karten aufgedeckt! Die anderen Spieler haben noch einen letzten Zug."

                    print(f"[DEBUG] Rundenende gestartet: {trigger_name} hat alle Karten aufgedeckt!")
                    ...
                    # Alle aufgedeckten Karten anzeigen
                    s.aufgedeckt_matrizen = msg["aufgedeckt_matrizen"]

                    # Punktzahlen anzeigen
                    s.final_scores = final_scores
                    s.show_final_scores = True

                    # Status aktualisieren
                    s.status_message = f"Runde beendet! {trigger_name} hat das Rundenende ausgelöst."
                    print(f"[DEBUG] Rundenende mit Punktzahlen: {final_scores}")
