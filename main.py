...
def main():
    ...
    while s.running:
        ...
        if s.game_mode == "host":
            ip_text = s.small_font.render(f"Deine IP: {c.get_local_ip()}", True, (0, 0, 0))
            s.screen.blit(ip_text, (10, 10))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if s.game_mode is None:
                    ...
                    if s.host_button.collidepoint(event.pos):
                        s.game_mode = "host"
                        s.waiting_for_players = True
                        print("[DEBUG] Game mode set to 'host'")
                    elif s.join_button.collidepoint(event.pos):
                        s.game_mode = "join"
                    ...
                    for i, button in enumerate(s.player_count_buttons):
                        if button.collidepoint(event.pos):
                            s.player_count = i + 1
                            print(f"[DEBUG] Player count set to {s.player_count}")
                            s.sock, s.spieler_id = c.connect_to_server()
                            serv.send_data(s.sock, {"serv.anzahl_spieler": s.player_count})
                    ...
                    for i, button in enumerate(s.round_count_buttons):
                        if button.collidepoint(event.pos):
                            s.round_count = i + 1
                    ...
                    if s.ip_input_box.collidepoint(event.pos):
                        s.active = True
                    ...
                    if s.input_box.collidepoint(event.pos):
                        s.active = True
        ...
        if s.game_mode is None:
            pygame.draw.rect(s.screen, (0, 200, 0), s.host_button)
            pygame.draw.rect(s.screen, (200, 0, 0), s.join_button)

            host_text = s.font.render("Spiel Hosten", True, (255, 255, 255))
            join_text = s.font.render("Spiel Beitreten", True, (255, 255, 255))

            s.screen.blit(host_text, (s.host_button.x + 20, s.host_button.y + 10))
            s.screen.blit(join_text, (s.join_button.x + 20, s.join_button.y + 10))

        # Spieleranzahl auswählen (Host)
        if s.waiting_for_players:
            headline = s.font.render("Spieleranzahl wählen", True, (0, 0, 0))
            s.screen.blit(headline, (s.screen.get_width() // 2 - headline.get_width() // 2, 200))
            total_width = len(s.player_count_buttons) * 60 - 10
            start_x = s.screen.get_width() // 2 - total_width // 2
            y = 260
            for i, button in enumerate(s.player_count_buttons):
                button.x = start_x + i * 60
                button.y = y
                pygame.draw.rect(s.screen, (0, 0, 255), button)
                player_text = s.small_font.render(f"{i+1}", True, (255, 255, 255))
                s.screen.blit(player_text, (button.x + button.width // 2 - player_text.get_width() // 2, button.y + 10))

        # Rundenzahl auswählen (Host)
        if s.waiting_for_rounds:
            headline = s.font.render("Rundenanzahl wählen", True, (0, 0, 0))
            s.screen.blit(headline, (s.screen.get_width() // 2 - headline.get_width() // 2, 200))
            total_width = len(s.round_count_buttons) * 60 - 10
            start_x = s.screen.get_width() // 2 - total_width // 2
            y = 260
            for i, button in enumerate(s.round_count_buttons):
                button.x = start_x + i * 60
                button.y = y
                pygame.draw.rect(s.screen, (255, 140, 0), button)
                round_text = s.small_font.render(f"{i+1}", True, (255, 255, 255))
                s.screen.blit(round_text, (button.x + button.width // 2 - round_text.get_width() // 2, button.y + 10))

        # Namenseingabe
        if s.waiting_for_name:
            prompt_text = s.small_font.render(f"Bitte Namen eingeben:", True, (0, 0, 0))
            s.screen.blit(prompt_text, (150, 270))
            txt_surface = s.font.render(s.text_input, True, (0, 0, 0))
            s.screen.blit(txt_surface, (s.input_box.x + 5, s.input_box.y + 5))
            pygame.draw.rect(s.screen, (0, 0, 0), s.input_box, 2)

        # IP Eingabe beim Join
        if s.game_mode == "join" and s.entering_ip:
            prompt = s.small_font.render("Host-IP eingeben:", True, (0, 0, 0))
            s.screen.blit(prompt, (150, 330))
            ip_surface = s.font.render(s.ip_input, True, (0, 0, 0))
            s.screen.blit(ip_surface, (s.ip_input_box.x + 5, s.ip_input_box.y + 5))
            pygame.draw.rect(s.screen, (0, 0, 0), s.ip_input_box, 2)

        # Statusmeldung anzeigen
        if s.status_message:
            status_surface = s.font.render(s.status_message, True, (0, 0, 0))
            s.screen.blit(status_surface, (s.screen.get_width() // 2 - status_surface.get_width() // 2, 100))
